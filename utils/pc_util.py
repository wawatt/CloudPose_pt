...
def random_sampling(pc, num_sample, replace=None, return_choices=False):
    ...
    if replace is None:
        replace = pc.shape[0] < num_sample  # false
...
def bbox_corners_center_measure(crnr1, crnr2, cls1=None, cls2=None):
    ...
    """
    # if cls1 == cls2:
    dist = np.linalg.norm(np.mean(crnr2, axis=0) - np.mean(crnr1, axis=0))
...
def bbox_corner_dist_measure(crnr1, crnr2):
    ...
    #     if d_ < dist:
    #         dist = d_
    dist = np.linalg.norm(crnr2 - crnr1, axis=1).sum() / 8.0
    u = sum([np.linalg.norm(x[0, :] - x[6, :]) for x in [crnr1, crnr2]]) / 2.0
...
def write_oriented_bbox(scene_bbox, out_filename):
    ...
    def convert_oriented_box_to_trimesh_fmt(box):
        ...
        trns[0:3, 3] = ctr
        trns[3, 3] = 1.0
        trns[0:3, 0:3] = box[6:15].reshape((3, 3))
        box_trimesh_fmt = trimesh.creation.box(lengths, trns)
        return box_trimesh_fmt
